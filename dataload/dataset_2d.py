...
class dataset_iitnn(Dataset):
    def __init__(self, data_dir, input1, input2, augmentation1, normalize_1, normalize_2, sup=True,
        ...
        for image in os.listdir(image_dir_1):
            ...
            if sup:
                mask_name = os.path.splitext(image)[0] + '.bmp'
...
class dataset_iitnnn(Dataset):
    def __init__(self, data_dir, input1, input2, input3, augmentation1, normalize_1, normalize_2, normalize_3, sup=True,
                 num_images=None, **kwargs):
        super(dataset_iitnnn, self).__init__()
        ...
        for image in os.listdir(image_dir_1):
            ...
            image_path_3 = os.path.join(image_dir_3, image)
            img_paths_3.append(image_path_3)

            if sup:
                mask_name = os.path.splitext(image)[0] + '.png'
                mask_path = os.path.join(mask_dir, mask_name)
                mask_paths.append(mask_path)

        assert len(img_paths_1) == len(img_paths_2) == len(img_paths_3)
        if sup:
            assert len(img_paths_1) == len(mask_paths)
        ...
        if num_images is not None:
            ...
            if num_images <= len_img_paths:
                img_paths_1 = img_paths_1[:num_images]
                img_paths_2 = img_paths_2[:num_images]
                img_paths_3 = img_paths_3[:num_images]
            else:
                rand_indices = torch.randperm(len_img_paths).tolist()
                new_indices = rand_indices[:remainder]
                ...
                img_paths_2 = img_paths_2 * quotient
                img_paths_2 += [img_paths_2[i] for i in new_indices]
                img_paths_3 = img_paths_3 * quotient
                img_paths_3 += [img_paths_3[i] for i in new_indices]
    ...
    def __getitem__(self, index):
        ...
        img_path_3 = self.img_paths_3[index]
        ...
        if self.sup:
            ...
            augment_1 = self.augmentation_1(image=img_1, image2=img_2, image3=img_3, mask=mask)
            img_1 = augment_1['image']
            img_2 = augment_1['image2']
            img_3 = augment_1['image3']
            mask = augment_1['mask']
            ...
            normalize_3 = self.normalize_3(image=img_3)
            img_3 = normalize_3['image']

            sampel = {'image': img_1, 'image_2': img_2, 'image_3': img_3, 'mask': mask, 'ID': os.path.split(mask_path)[1]}

        else:
            augment_1 = self.augmentation_1(image=img_1, image2=img_2, image3=img_3)
            img_1 = augment_1['image']
            img_2 = augment_1['image2']
            img_3 = augment_1['image3']
            ...
            sampel = {'image': img_1, 'image_2': img_2, 'image_3': img_3, 'ID': os.path.split(img_path_1)[1]}
...
def imagefloder_iitnnn(data_dir, input1, input2, input3, data_transform_1, data_normalize_1, data_normalize_2, data_normalize_3, sup=True, num_images=None, **kwargs):
    dataset = dataset_iitnnn(data_dir=data_dir,
                           input1=input1,
                           input2=input2,
                           input3=input3,
                           augmentation1=data_transform_1,
