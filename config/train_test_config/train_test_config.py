...
def print_train_loss_DWE(train_loss_sup_1, train_loss_sup_2, train_loss_cps, train_loss, num_batches, print_num, print_num_half):
    train_epoch_loss_sup1 = train_loss_sup_1 / num_batches['train_sup']
...
def print_train_eval_DWE(num_classes, score_list_train1, score_list_train2, mask_list_train, print_num):
    ...
    if num_classes == 2:
        if best_list[1] < eval_list[1]:
            ...
            for i in range(len(name_list_val)):
                color_results = np.where(pred_results[i] != 0, 255, 0).astype(np.uint8)
                Image.fromarray(color_results).save(os.path.join(path_seg_results, name_list_val[i]))
...
def save_test_2d(num_classes, score_list_test, name_list_test, threshold, path_seg_results, palette):
    ...
    if num_classes == 2:
        ...
        for i in range(len(name_list_test)):
            color_results = np.where(pred_results[i] != 0, 255, 0).astype(np.uint8)
            Image.fromarray(color_results).save(os.path.join(path_seg_results, name_list_test[i]))
